...
class NoisyHttpRequestsFilter(logging.Filter):
    """Filter noisy HTTP request log entries.

    This filter prevents common low-value HTTP request entries from cluttering
    logs. PATCH requests are often generated by the h2o-wave application, and
    Hugging Face Hub model metadata/tokenizer lookups can produce many repeated
    HEAD/GET requests during model loading.
    """

    _NOISY_HTTP_REQUEST_PATTERNS = (
        re.compile(r"HTTP Request: PATCH"),
        re.compile(r"HTTP Request: (?:HEAD|GET) https://huggingface\.co/"),
    )

    def filter(self, record):
        log_message = record.getMessage()
        return not any(
            pattern.search(log_message)
            for pattern in self._NOISY_HTTP_REQUEST_PATTERNS
        )
...
def initialize_logging(cfg: Any | None = None):
    ...
    if (cfg is None) or (cfg.environment._local_rank == 0) or cfg.logging.log_all_ranks:
        ...
        console_handler.addFilter(NoisyHttpRequestsFilter())
    ...
    if file_handler is not None:
        file_handler.addFilter(NoisyHttpRequestsFilter())
